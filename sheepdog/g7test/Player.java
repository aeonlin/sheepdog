...
public class Player extends sheepdog.sim.Player {
    ...
    public Point move(Point[] dogs, // positions of dogs
        ...
        if( mode == false) {
            ...
            //return squeeze_strategy(dogs, sheeps, 2.58, max_sheep_speed * 0.588);
            double spr;
            if(!globalRecord.initialized) {
                globalRecord.initialize(id, dogs, sheeps);
            }
            spr = globalRecord.max_speed_for_gap_width(2.58);
            //System.out.println("SPR = " + spr);
            return dog_gap_test(dogs, sheeps, 2.58, max_sheep_speed * spr);
    ...
    private Point squeeze_strategy(Point[] dogs, Point[] sheeps, double gapLength, double dogSpeed) {
        System.out.println("S-phase:" + strategy_phase);
        switch (strategy_phase) {
            case -1: // move to right
                return move_dogs_to_the_other_side(dogs, sheeps);
            case 0: // move all dogs to up and down
                return move_dogs_to_up_down(dogs, sheeps, gapLength);
            case 1:
                return move_dogs_center(dogs, sheeps, gapLength, dogSpeed, 1.0);
    ...
    private Point move_dogs_to_up_down(Point[] dogs, Point[] sheeps, double gapLength) {
        Point next = new Point ();
        if (dogs[id-1].x == 50.0 && dogs[id-1].y != 0.0 && dogs[id-1].y != 100.0) {
            System.out.println("ON LEFT");
            if (id <= dogs.length / 2) {
                if (dogs[id-1].y >= max_dog_speed) {
                    next = new Point(dogs[id-1].x, dogs[id-1].y - max_dog_speed);
                }
                else {
                    next = new Point(dogs[id-1].x, 0.0);
                }
            }
            else {
                if (dogs[id-1].y <= 100.0 - max_dog_speed ) {
                    next = new Point(dogs[id-1].x, dogs[id-1].y + max_dog_speed);
                }
                else {
                    next = new Point(dogs[id-1].x, 100.0);
                }
            }
        }
        if (dogs[id-1].y == 0.0 || dogs[id-1].y == 100.0) {
            System.out.println("TWO-ENDS");
            if(dogs[id-1].x <= 100.0 - max_dog_speed) {
                next = new Point(dogs[id-1].x + max_dog_speed, dogs[id-1].y);
            }
            else if(dogs[id-1].x < 100.0) {
                next = new Point(100.0, dogs[id-1].y);
            }
            else {
                if(dogs[id-1].y > 50.0) {
                    next = new Point(dogs[id-1].x, dogs[id-1].y - max_dog_speed);
        ...
        if (dogs[id-1].x == 100.0 ) {
            System.out.println("ON RIGHT");
            boolean checkDogs = true;
            Point goal = new Point (100.0, gapLength * ((double)(id) - (double)(dogs.length)/2.0 - 0.5) + 50.0);
            ...
            for (int i = 0; i < dogs.length; i++) {
                ...
                if( dogs[i].y != gapLength * ((double)(i+1) - (double)(dogs.length)/2.0 - 0.5) + 50.0) {
                    checkDogs = false;
                    break;
                }
            }
            ...
            next = next_toward_goal(dogs[id-1], goal, max_dog_speed);
        }
        return next;
    }

    private Point dog_gap_test(Point[] dogs, Point[] sheeps, double gapLength, double dogSpeed) {
        System.out.println("S-phase:" + strategy_phase);
        switch (strategy_phase) {
            case -1: // move to right
                return move_dogs_to_the_other_side(dogs, sheeps);
            case 0: // move all dogs to right
                return move_dogs_to_end(dogs, sheeps, gapLength);
            case 1:
            //    return move_dogs_vshape(dogs, sheeps, gapLength);
            case 2: // sweep back!
                return move_dogs_back_left(dogs, sheeps, gapLength, dogSpeed);
                //return move_dogs_back_left_zigzag(dogs, sheeps, gapLength, dogSpeed);
            case 3:
                return move_dogs_center(dogs, sheeps, gapLength, dogSpeed, 0.0);
    ...
    private Point move_dogs_center(Point[] dogs, Point[] sheeps, double gapLength, double dogSpeed, double slowRatio) {
        Point goal = new Point(dogs[id-1].x, 50.0);
        Point next = new Point(dogs[id-1].x, dogs[id-1].y);

        double speedRatio = speed_ratio(dogs.length, id, slowRatio);

        if (dogs[0].y >= 48.0 || dogs[dogs.length-1].y <= 52.0) {
            strategy_phase = 4;
        }
        else {
            next = next_toward_goal(next, goal, dogSpeed * speedRatio);
        }
        return next;
    }

    private double speed_ratio( int length, int i, double slowRatio) {
        // 1 <= i <= length
        ...
        if( id <= n / 2) {
            speedRatio = 1.0 - (1.0 - slowRatio) * (double)(id-1)/(double)(numInterval);
        }
        else {
            speedRatio = slowRatio + (1.0 - slowRatio) * (double)(id - n/2 -1) / (double)(numInterval);
        }
        return speedRatio;
    }

    private Point move_dogs_back_left_zigzag(Point[] dogs, Point[] sheeps, double gapLength, double dogSpeed) {
        double speedRatio = speed_ratio(dogs.length, id, 1.0);
        Point goal = new Point(dogs[id-1].x - dogSpeed * speedRatio, dogs[id-1].y);
        Point next = new Point(dogs[id-1].x, dogs[id-1].y);
        if(globalRecord.sweepPhase != -1) {
            goal.y -= gapLength/2.0;
        ...
        */
        if (dogs[0].x == 50.0 || dogs[dogs.length-1].x == 50.0) {
            strategy_phase = 3;
        }
        else {
    ...
    private Point move_dogs_back_left(Point[] dogs, Point[] sheeps, double gapLength, double dogSpeed) {
        Point goal = new Point(50.0, dogs[id-1].y);
        Point next = new Point(dogs[id-1].x, dogs[id-1].y);

        double speedRatio = speed_ratio(dogs.length, id, 0.5);
        /*
        if (next.equals(goal)) {
            strategy_phase = 3;
        }
    ...
    private Point move_dogs_vshape(Point[] dogs, Point[] sheeps, double gapLength) {
        boolean checkDogs = true;
        Point next = new Point(dogs[id-1].x, dogs[id-1].y);
        for(int i = 0; i <dogs.length; i++) {
            if ( i % 2 == 1 ) {
                if (dogs[i].x != 100.0 - gapLength) {
                    checkDogs = false;
                    break;    
                }
            }
        ...
        if( id % 2 == 0) {
            Point goal = new Point(100.0 - gapLength, dogs[id-1].y);
            next = next_toward_goal(next, goal, 0.1);
        }
        if( checkDogs ) {
            strategy_phase = 2;
        }
    ...
    private Point move_dogs_to_end(Point[] dogs, Point[] sheeps, double gapLength) {
        Point next = new Point ();
        if (dogs[id-1].x == 50.0 && dogs[id-1].y != 0.0 && dogs[id-1].y != 100.0) {
            System.out.println("ON LEFT");
            if (id <= dogs.length / 2) {
